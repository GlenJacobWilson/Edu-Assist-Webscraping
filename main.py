...
from models import (User, UserCreate, Token, PinnedNotification,
                    Question, Answer, StudyGroup, NotifSubscription, FCMToken)
...
from notifications import start_scheduler, send_push, poll_and_notify
...
# FIX: proper CORS — no wildcard, credentials enabled
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://172.30.12.235:3000",
        "https://unruly-unfailing-spirited.ngrok-free.dev",
    ],
...
@app.get("/")
def home():
    return {"message": "KTU EduAssist API v2.0"}
...
def get_pins(email: str = Depends(get_current_user_email), session: Session = Depends(get_session)):
    return session.exec(select(PinnedNotification.notification_id).where(
        PinnedNotification.user_email == email)).all()


@app.post("/pin/{nid}")
def pin(nid: int, email: str = Depends(get_current_user_email), session: Session = Depends(get_session)):
    if not session.exec(select(PinnedNotification).where(
            PinnedNotification.user_email == email,
            PinnedNotification.notification_id == nid)).first():
        session.add(PinnedNotification(user_email=email, notification_id=nid))
        session.commit()
...
def unpin(nid: int, email: str = Depends(get_current_user_email), session: Session = Depends(get_session)):
    for row in session.exec(select(PinnedNotification).where(
            PinnedNotification.user_email == email,
            PinnedNotification.notification_id == nid)).all():
        session.delete(row)
    session.commit()
    ...
    for q in qs:
        ...
        result.append({
            ...
            "answers": [{"id": a.id, "question_id": a.question_id, "user_name": a.user_name,
                         "content": a.content, "timestamp": a.timestamp,
                         "is_best": bool(a.is_best)} for a in answers]
        })
    return result


@app.post("/forum/question")
def post_question(data: dict, email: str = Depends(get_current_user_email),
...
def post_answer(q_id: int, data: dict, email: str = Depends(get_current_user_email),
                session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
...
def toggle_solved(q_id: int, email: str = Depends(get_current_user_email),
                  session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
    q = session.get(Question, q_id)
    if not q:
        raise HTTPException(status_code=404)
    if q.user_name != user.full_name and not user.is_admin:
...
def mark_best(a_id: int, email: str = Depends(get_current_user_email),
              session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
    ...
    if helper:
        helper.helper_points = (helper.helper_points or 0) + 5
        session.add(helper)
    if q:
        q.is_solved = True
        session.add(q)
...
def delete_question(q_id: int, email: str = Depends(get_current_user_email),
                    session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
...
def leaderboard(session: Session = Depends(get_session)):
    ...
    return [{"name": u.full_name, "points": u.helper_points or 0,
             "semester": u.semester, "department": u.department}
            for u in users if (u.helper_points or 0) > 0]
...
def get_study_groups(semester: str = None, session: Session = Depends(get_session)):
    ...
    if semester:
        q = q.where(StudyGroup.semester == semester)
    return session.exec(q).all()


@app.post("/studygroups")
def create_group(data: dict, email: str = Depends(get_current_user_email),
                 session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
...
def delete_group(sg_id: int, email: str = Depends(get_current_user_email),
                 session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.email == email)).first()
    sg = session.get(StudyGroup, sg_id)
    if not sg:
        raise HTTPException(status_code=404)
    if sg.created_by != user.full_name and not user.is_admin:
        raise HTTPException(status_code=403)
    session.delete(sg)
    session.commit()
    return {"status": "deleted"}
...
def get_sub(email: str = Depends(get_current_user_email), session: Session = Depends(get_session)):
    ...
    return {"whatsapp": sub.whatsapp if sub else "",
            "notify_email": sub.notify_email if sub else "",
            "urgent_only": sub.urgent_only if sub else True}


@app.post("/subscribe")
def subscribe(data: dict, email: str = Depends(get_current_user_email),
              session: Session = Depends(get_session)):
    sub = session.exec(select(NotifSubscription).where(NotifSubscription.user_email == email)).first()
    if sub:
        sub.whatsapp = data.get("whatsapp", "")
        sub.notify_email = data.get("notify_email", "")
        sub.urgent_only = data.get("urgent_only", True)
        session.add(sub)
    else:
        session.add(NotifSubscription(user_email=email, whatsapp=data.get("whatsapp", ""),
                                      notify_email=data.get("notify_email", ""),
                                      urgent_only=data.get("urgent_only", True)))
    session.commit()
...
async def call_claude(prompt: str, system: str = "") -> str:
    if not ANTHROPIC_API_KEY:
        raise HTTPException(status_code=503, detail="AI features require ANTHROPIC_API_KEY in .env file")
    ...
    if res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"AI error: {res.text}")
...
async def summarize(data: dict):
    ...
    result = await call_claude(prompts.get(mode, prompts["summarize"]),
                               "You are a KTU exam preparation assistant.")
    return {"result": result}


@app.get("/materials")
def get_materials(semester: str = "S6", branch: str = "CSE", material_type: str = "qp"):
    return get_subject_list(semester, branch, material_type) or []
...
def register_token(data: dict, email: str = Depends(get_current_user_email),
                   session: Session = Depends(get_session)):
    token = data.get("token", "").strip()
    if not token:
        raise HTTPException(status_code=400, detail="Token required")
    existing = session.exec(select(FCMToken).where(FCMToken.token == token)).first()
    if not existing:
        session.add(FCMToken(user_email=email, token=token))
        session.commit()
    return {"status": "registered"}


@app.post("/notify/test")
def test_notification(email: str = Depends(get_current_user_email),
                      session: Session = Depends(get_session)):
    tokens = session.exec(
        select(FCMToken.token).where(FCMToken.user_email == email)
    ).all()
    if not tokens:
        raise HTTPException(status_code=404,
                            detail="No device registered. Open the app on your phone and log in first.")
    send_push(
        title="🔔 KTU Edu-Assist — Test",
        body="Push notifications are working! You'll get real alerts when KTU posts something new.",
        tokens=list(tokens),
    )
